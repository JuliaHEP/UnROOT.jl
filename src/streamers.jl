...
# Name resolution helpers for ROOT class names → Julia identifiers.
# safename: maps C++ namespace separator "::" → "_3a3a_"  (used here and in root.jl)
# aliasfor:  maps known aliases to their Julia type          (defined in types.jl)
...
function Streamers(io)
    ...
    for i ∈ 1:size
        ...
        # Each object in the list is followed by a trailing byte that encodes
        # how many additional bytes to skip (typically 0). This is part of
        # ROOT's TBufferFile object-stream framing, not simply a 1-byte pad.
...
function topological_sort(streamer_infos)
    ...
    provided = Set{String}()
    sorted_streamer_infos = StreamerInfo[]
    while length(streamer_infos) > 0
        remaining_items = StreamerInfo[]
        emitted = false
        ...
        if !emitted
            # Circular dependency — force-emit all remaining items and break.
            @debug "  circular dependency detected among: $(map(s -> s.streamer.fName, streamer_infos))"
            for streamer_info in streamer_infos
                if aliasfor(streamer_info.streamer.fName) === nothing
                    push!(sorted_streamer_infos, streamer_info)
                end
...
function readobjany!(io, tkey::TKey, refs)
    ...
    # TTree objects are streamed from a datastream whose origin is the start of
    # the decompressed basket, not the file position used by origin(tkey). This
    # causes the relative offsets calculated above to be negative. Compensate by
    # shifting by the key header size and its absolute file position.
...
function parsefields!(io, fields, T::Type{TStreamerElement})
    ...
    # Compatibility shim: some older ROOT files serialise Bool_t fields with type
    # code 11 (kUChar) instead of the correct 18 (kBool). Remap when the typename
...
abstract type AbstractTStreamerSTL end

@tstreamer_element_struct mutable struct TStreamerSTL <: AbstractTStreamerSTL
    fSTLtype
    fCtype
end

@tstreamer_element_struct mutable struct TStreamerSTLstring <: AbstractTStreamerSTL
    fSTLtype
    fCtype
end

function unpack(io, tkey::TKey, refs::Dict{Int32, Any}, ::Type{T}) where T <: AbstractTStreamerSTL
    @initparse
...
# TStreamerArtificial is injected by TSchemaRule for schema evolution; it is
# transient-only (ROOT source: TStreamerElement.h) and carries no persistent data
# beyond the base TStreamerElement fields. Read and discard it gracefully.
@tstreamer_element_struct mutable struct TStreamerArtificial end

function unpack(io, tkey::TKey, refs::Dict{Int32, Any}, T::Type{TStreamerArtificial})
    @initparse
    preamble = Preamble(io, T)
    parsefields!(io, fields, TStreamerElement)
    endcheck(io, preamble)
...
# Resolve the versioned concrete struct for a ROOTStreamedObject abstract type
# by reading the preamble version from io. Errors with a clear message if the
# version has no corresponding bootstrapped struct (e.g. a newer ROOT version).
function _versioned_streamer(io, ::Type{T}) where {T<:ROOTStreamedObject}
    preamble = Preamble(io, T)
    basename = last(split(string(T), "."))
    versioned_name = Symbol(basename, "_$(preamble.version)")
    if !isdefined(@__MODULE__, versioned_name)
        error("Unknown version $(preamble.version) of ROOT type $basename. " *
              "Consider updating UnROOT.jl or filing an issue.")
    end
    preamble, getfield(@__MODULE__, versioned_name)
end
